...
RProgram  rProgram[R_NUM_PROGRAM];
RModel    rModel;
RViewport rViewport;
RView     rView;
...
static bool Setup() {
	...
	//ambient
	if (!R_CreateProgram(L"002/ambient_vs.txt", L"002/ambient_fs.txt", rProgram[R_PROGRAM_AMBIENT])) {
		return false;
	}
	...
	if (!R_LinkProgram(rProgram[R_PROGRAM_AMBIENT])) {
		R_DestroyProgram(rProgram[R_PROGRAM_AMBIENT]);
		return false;
	}
	...
	//lighting
	if (!R_CreateProgram(L"002/lighting_vs.txt", L"002/lighting_fs.txt", rProgram[R_PROGRAM_LIGHTING])) {
		return false;
	}
	...
	if (!R_LinkProgram(rProgram[R_PROGRAM_LIGHTING])) {
		R_DestroyProgram(rProgram[R_PROGRAM_LIGHTING]);
		return false;
	}
	...
	//load textures
	rTextures[0] = R_CreateTexture(L"StoneWall2.bmp", true);
	rTextures[1] = R_CreateTexture(L"Tiles.bmp", true);
	rTextures[2] = R_CreateTexture(L"brick2.bmp", true);
	rTextures[3] = R_CreateTexture(L"StoneWallBump.bmp", true);
	rTextures[4] = R_CreateTexture(L"TilesBump.bmp", true);
	rTextures[5] = R_CreateTexture(L"brick2Bump.bmp", true);
	rTextures[6] = R_CreateTexture(L"LightMap\\Map2_LightMap0.bmp", true);
	rTextures[7] = R_CreateTexture(L"LightMap\\Map2_LightMap1.bmp", true);
	rTextures[8] = R_CreateTexture(L"Particle.bmp", true);
	...
	rModel.vb = R_CreateVertexBuffer(sizeof(Vertex), model.nVertices, (Vertex*)model.vertices);
	rModel.ib = R_CreateIndexBuffer(sizeof(uint16), model.nIndices, (uint16*)model.indices);

	for (uint32 i = 0; i < model.nBatches; ++i) {
	...
	//light model
	rLightModel[0].vb = R_CreateVertexBuffer(sizeof(LightModelVertex), 4, nullptr);
	rLightModel[1].vb = R_CreateVertexBuffer(sizeof(LightModelVertex), 4, nullptr);
...
static void DrawAmbient(const glm::mat4 &mvpMatrix) {
	...
	for (int i = 0; i < rModel.nBatchs; ++i) {
		...
		glDrawElements(GL_TRIANGLES, rBatch->nIndices, GL_UNSIGNED_SHORT, (const GLvoid*)(rBatch->firstIndex * sizeof(uint16)));
	}
...
void DrawLightModel(const glm::mat4 &mvpMatrix) {
	...
	for (int l = 0; l < LIGHT_COUNT; ++l) {
		...
		LightModelVertex * v = (LightModelVertex*)R_MapVertexBuffer();
		{
...
static void OnDisplay() {
	...
	R_PerspectiveMatrix(rViewport, 45.0f, projectionMatrix);

	glm::mat4 viewMatrix;
	R_ViewMatrix(rView, viewMatrix);
